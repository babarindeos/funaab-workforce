    ...
    <div class="flex flex-col border-0 w-[95%]  mx-auto">
        ...
        <section class="flex flex-col w-full md:w-full py-6 px-2 md:px-4 border-0 border-red-900 mx-auto border border-1">
            ...
            <div class="flex border-b border-gray-300 py-2 justify-between">
                    <div class='flex'>
                            <h1 class="text-2xl font-semibold text-gray-800">Classes of Degrees</h1>
                        
                    </div>
                    <div>

                            <a href="{{ route('admin.classes_of_degrees.create') }}" class="border border-green-600 text-green-600 py-2 px-6 
                                            rounded-lg text-xs md:text-sm hover:bg-green-600 hover:text-white hover:border-green-600">Create Class of Degree</a>
                    </div>
        ...
        @if ($classes_of_degrees->count())
                    <div class='flex flex-col w-full md:flex-row md:items-center'>
                        <div class='py-1 text-xl font-medium md:w-1/2'>Classes of Degrees ({{ $classes_of_degrees->count() }})</div>

                        <!-- Search //-->
                        <div class='flex flex-col gap-2 w-full py-2 md:py-2 md:w-1/2 md:justify-end md:items-end border-0'>
                                <form action="{{ route('admin.classes_of_degrees.index') }}" method="GET" class="flex flex-row items-center w-full border-0">
                    ...
                    <table class="table-auto border-collapse border border-1 border-gray-200 w-[100%]">
                        ...
                        <tbody>
                            @php
                                $counter = 0;
                            @endphp
                            @foreach($classes_of_degrees as $class_of_degree)
                            <tr class="border border-b border-gray-200 ">
                                ...
                                <td class="py-4 pr-4">
                                    ...
                                    <div class='py-1'>
                                            ...
                                            <a class="font-semibold text-blue-800 underline" href="{{ route('admin.classes_of_degrees.show',['class_of_degree'=>$class_of_degree->id]) }}">
                                                {{ $class_of_degree->degree_name }}
                                            </a>                             
                                    </div>
                                ...
                                <td>
                                                <span class="text-sm">
                                                            <a href="{{ route('admin.degrees.edit',['class_of_degree' => $class_of_degree->id]) }}" class="hover:bg-blue-500 bg-blue-400 text-white rounded 
                                                                px-4 py-1 text-xs" href="#">Edit</a>
                                                </span>
                                           
                                                <span class="text-sm">
                                                        <a class="hover:bg-red-500 bg-red-400 text-white rounded 
                                                            px-4 py-1 text-xs" href="{{ route('admin.degrees.confirm_delete',['class_of_degree' => $class_of_degree->id]) }}">Delete</a>
                                                </span>
                                </td>
            ...
            @else
                    <section class="flex flex-col w-[95%] md:w-[95%] mx-auto px-0 py-8 border-0">
                        <div class="flex flex-row border-0 justify-center text-2xl font-semibold text-gray-300">
                                There is currently no Class of Degree
                        </div>
                    </section>
            @endif
