    ...
    <div class="flex flex-col border-0 w-[95%]  mx-auto">
        ...
        <section class="flex flex-col w-full md:w-full py-6 px-2 md:px-4 border-0 border-red-900 mx-auto border border-1">
            ...
            <div class="flex border-b border-gray-300 py-2 justify-between">
                    <div class='flex'>
                            <h1 class="text-2xl font-semibold text-gray-800">Deployment</h1>
                        
                    </div>
                    <div>

                            <a href="{{ route('admin.deployments.create') }}" class="border border-green-600 text-green-600 py-2 px-6 
                                            rounded-lg text-xs md:text-sm hover:bg-green-600 hover:text-white hover:border-green-600">Create Deployment</a>
                    </div>
        ...
        @if ($deployments->count())
                    <div class='flex flex-col w-full md:flex-row md:items-center'>
                        <div class='py-1 text-xl font-medium md:w-1/2'>Deployments ({{ $deployments->count() }})</div>

                        <!-- Search //-->
                        <div class='flex flex-col gap-2 w-full py-2 md:py-2 md:w-1/2 md:justify-end md:items-end border-0'>
                                <form action="{{ route('admin.deployments.index') }}" method="GET" class="flex flex-row items-center w-full border-0">
